...
VOID WINAPI ReceiveMessageHandle(LPVOID lphPipe)
{
	...
	} while (lpsReadHeader[0] != OP_INCOMING_ADMIN_CHAT_MESSAGE_EXIT);
...
VOID WINAPI SendMessagesHandle(LPVOID lpParams)
{
	...
	do
	{
		...
		fgets(lpsChatMessageBuffer, BUFFER_SIZE, stdin);
	...
	lpsWriteHeader[1] = dwMessageToSendSize + 1;
	lpsWriteHeader[2] = 0;
...
void main(int argc, char** argv)
{
	...
	strcat_s(lpsChatName, BUFFER_SIZE, argv[1]);
	strcat_s(lpsChatName, BUFFER_SIZE, "-");
	strcat_s(lpsChatName, BUFFER_SIZE, argv[2]);

	printf_s("Welcome to %s chat %s!\n", argv[1], lpsUsername);
	
	if (OpenBothPipes(lpsChatName, &hPipeInStream, &hPipeOutStream) != 0)
	{
	...
	hThreadsIO[1] = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)ReceiveMessageHandle, hPipeOutStream, 0, NULL);
